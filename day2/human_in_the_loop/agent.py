...
load_dotenv()
GITHUB_TOKEN = os.getenv("GOOGLE_API_KEY")
...
async def debug_run(session_service: InMemorySessionService, shipping_runner: Runner):
    # Demo 1: It's a small order. Agent receives auto-approved status from tool
    await run_shipping_workflow(session_service, shipping_runner,"Ship 3 containers to Singapore")

    # Demo 2: Workflow simulates human decision: APPROVE ✅
    await run_shipping_workflow(session_service, shipping_runner, "Ship 10 containers to Rotterdam", auto_approve=True)

    # Demo 3: Workflow simulates human decision: REJECT ❌
    await run_shipping_workflow(session_service, shipping_runner, "Ship 8 containers to Los Angeles", auto_approve=False)
...
if __name__ == '__main__':
    ...
    asyncio.run(debug_run(session_service, shipping_runner))
